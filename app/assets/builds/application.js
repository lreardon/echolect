  ...
  var reflexes = new Proxy({}, {
    get: function(target, prop) {
      if (stages.includes(prop))
        return Object.fromEntries(Object.entries(target).filter(([_, reflex]) => reflex.stage === prop));
      else if (prop === "last")
        return lastReflex;
      else if (prop === "all")
        return target;
...
cable_ready npm: ${global.version}`;
  ...
  var afterDOMUpdate = (event) => {
    ...
    if (reflex.piggybackOperations.length)
      global.perform(reflex.piggybackOperations);
  };
        ...
        try {
          ...
          this.mediaRecorder.ondataavailable = (event) => {
            ...
            reader.onloadend = () => {
              ...
              audio_channel_default.sendAudioData(base64data);
            };
